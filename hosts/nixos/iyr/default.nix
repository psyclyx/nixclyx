...
in
{
  ...
  # now this is validation-only: iyr pulls a lease and can source pings
  # out enp3s0.251 (e.g. `ping -I enp3s0.251 1.1.1.1`,
  # `ping -6 -I enp3s0.251 2606:4700:4700::1111`), but all of its routes
  # (v4 and v6 alike) live in a dedicated table (251) reached only by an
  ...
  #
  # IPv6 mirrors WAN1's shape (see 31-enp3s0.${transitVlan} generated by
  # derived/gateway.nix): accept RA for SLAAC + WithoutRA DHCPv6 solicit
  # for a leased address, plus a manual on-link ::/0 in case the modem's
  # RA has Router Lifetime=0 like WAN1's does. Both the RA-learned and
  # manual default routes are pinned to table 251 (RouteTable on the RA
  # config, Table= on the static route) so they can't leak into main.
  #
