{
  ...
  config =
    ...
    let
      ...
      hasConfigure = cfg.authPasswordFile != null || cfg.configureTokenFile != null || cfg.pki.enable;
    ...
    in
    {
      ...
      systemd.services.openbao-configure = lib.mkIf hasConfigure {
        ...
        ++ lib.optional cfg.autoInit.enable pkgs.age;
        script = ''
            set -euo pipefail
          ...
          # -- Phase 3: Run PKI + user configure script --
          ${pkiScript}
          ${cfg.configure}
