{
  ...
  config =
      ...
      mkCertService =
        ...
        let
          writeArgs =
            ...
            + " ttl=${cert.ttl}"
            + lib.optionalString (cert.altNames != [ ]) " alt_names=${lib.concatStringsSep "," cert.altNames}"
            + lib.optionalString (cert.ipSans != [ ]) " ip_sans=${lib.concatStringsSep "," cert.ipSans}"
            + lib.optionalString (cert.organization != null) " organization=${cert.organization}";
    ...
    {
      systemd.services =
        lib.mapAttrs' (name: c: lib.nameValuePair "openbao-pki-${name}" c.service) certConfigs
        // (lib.mkMerge (
          lib.flatten (
            lib.mapAttrsToList (
              certName: cert:
              map (unit: {
                "${lib.removeSuffix ".service" unit}" = {
                  after = [ "openbao-pki-${certName}.service" ];
                  wants = [ "openbao-pki-${certName}.service" ];
                };
              }) cert.reloadUnits
            ) cfg.certificates
          )
      ...
      systemd.timers = lib.mapAttrs' (
        name: c: lib.nameValuePair "openbao-pki-${name}" c.timer
      ) certConfigs;
    };
}
