{
  path = ["psyclyx" "nixos" "services" "openbao-kv"];
  ...
  config = {
    ...
    mkSecretService = name: secret: let
  ...
  in {
    systemd.services = lib.mapAttrs' (name: c:
      lib.nameValuePair "openbao-kv-${name}" c.service
    ) secretConfigs;
    systemd.timers = lib.mapAttrs' (name: c:
      lib.nameValuePair "openbao-kv-${name}" c.timer
    ) secretConfigs;
  };
}
