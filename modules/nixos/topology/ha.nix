    ...
    lib.mapAttrsToList (
      ...
      let
        ...
        idx = lib.lists.findFirstIndex (
          m: m == hostname
        ) (throw "${hostname} is not a member of group ${groupName}") ha.members;
      in
      {
        vip = ha.vip.ipv4;
    ...
    let
      ...
      mkFrontendBackend =
        ...
        let
          ...
          checkDirective =
            if svc.check != null || svc.mode == "http" then
              ''
                option httpchk
              ...
              "";
          checkPortSuffix = if svc.checkPort != null then " port ${toString svc.checkPort}" else "";
          sslSuffix =
            if svc.checkSsl && svc.mode == "http" then
              " ssl verify none check-ssl"
            else if svc.checkSsl then
      ...
      serviceSections = lib.concatStringsSep "" (
        lib.flatten (
          lib.mapAttrsToList (
            groupName: g:
...
in
{
  config = lib.mkIf hasGroups {
    ...
    services.keepalived = {
      enable = true;
      vrrpInstances = builtins.listToAttrs (
        lib.imap0 (
          i: vip:
