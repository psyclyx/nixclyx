...
in
{
  options = {
    psyclyx.hardware.drivers.usb = {
      ...
      storage = lib.mkEnableOption "USB storage" // {
        default = true;
      };
      xhci = lib.mkEnableOption "USB XHCI controller (USB 3, 2, 1)" // {
        default = true;
      };
      ehci = lib.mkEnableOption "USB EHCI controller (USB 2)" // {
        default = true;
      };
      uhci = lib.mkEnableOption "USB UHCI controller (USB 1)" // {
        default = true;
      };
    };
  };
