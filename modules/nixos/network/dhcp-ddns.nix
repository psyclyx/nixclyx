  ...
  in lib.mkIf (dhcpCfg.enable && dhcpCfg.pools != {}) {
    services.kea.dhcp-ddns = {
      ...
      settings = {
        ...
        # populated from egregore by topology/zones.nix:siteZone.
        # Each interface DDNS-registers under its per-VLAN zone via
        # the per-pool ddns-qualifying-suffix in topology/dhcp.nix.
